...
void get_accel_ms2(float *out) {
	...
	if (lsm6dsv80x_ctx.handle != NULL && ((SPI_HandleTypeDef*)lsm6dsv80x_ctx.handle)->State == HAL_SPI_STATE_READY){
		...
		HAL_SPI_TransmitReceive(lsm6dsv80x_ctx.handle, imu_tx_buf, imu_rx_buf, 13, 1);
		...
		} else if (lsm6dsv80x_ctx.handle == NULL && ((SPI_HandleTypeDef*)lsm6dsv80x_ctx.handle)->State == HAL_SPI_STATE_READY){
			return;
		}
}
...
//		if (lps22hh_ctx.handle != NULL && ((SPI_HandleTypeDef*)lps22hh_ctx.handle)->State == HAL_SPI_STATE_READY){
	...
	HAL_SPI_TransmitReceive(lps22hh_ctx.handle, baro_tx_buf, baro_rx_buf, 4, 1);
	HAL_GPIO_WritePin(BARO_CS_GPIO_Port, BARO_CS_Pin, 1); // deassert baro CS
	...
	} else if (lps22hh_ctx.handle == NULL && ((SPI_HandleTypeDef*)lps22hh_ctx.handle)->State == HAL_SPI_STATE_READY) {
			return;
		}
//		HAL_GPIO_WritePin(BARO_CS_GPIO_Port, BARO_CS_Pin, 1); // deassert baro CS
//		pres_raw = (uint32_t)(((uint32_t)baro_rx_buf[3] << 24) | ((uint32_t)baro_rx_buf[2] << 16) | ((uint32_t)baro_rx_buf[1] << 8));
